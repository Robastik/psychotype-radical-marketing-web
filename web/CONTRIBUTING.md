...
2. **При изменении контента справочника** убедитесь, что `build:guide` проходит без ошибок. Сгенерированные файлы (`src/app/guide/data/guide-data.ts`) не нужно коммитить — они исключены из Git.

### Редактирование методического справочника

Источник правды для справочника — Markdown-файлы в `docs/reference/Tasks/`:

- `doc-<КОД>.md` — текст документа
- Ссылки вида `[I.1.1]` автоматически превращаются в ссылки `/guide/I.1.1`
- После редактирования запустите `npm run build:guide`
